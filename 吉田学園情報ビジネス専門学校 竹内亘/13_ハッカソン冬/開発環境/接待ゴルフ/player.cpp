...
#define FILE_NAME						("data\\TEXT\\motionサラリーマン.txt")
...
//プレイヤークラス コンストラクタ
//--------------------------------------------
CPlayer::CPlayer() : CScene(4, CScene::OBJTYPE_MESHFIELD)
{
	m_bJump = false;
...
//オブジェクトの生成
//--------------------------------------------
CPlayer *CPlayer::Create(D3DXVECTOR3 pos, float fLength)
{
	//プレイヤーポインタ
	CPlayer *pPlayer;
	pPlayer = new CPlayer;
...
void CPlayer::Update(void)
{
	...
	{
		m_pos = D3DXVECTOR3(0, 0, 10500);
	}

#if 1
	if (m_State == PLAYERSTATE_NORMAL && CameraState == CCamera::STATE_FALL)
	{
		//任意のキーA
		...
		else if (pInputJoypad->GetStick(0) == true)
		{//移動
		 //モーション切り替え
			if (m_bJump == false)
			{
			...
			m_pos.x += sinf(Rot) * MOVE_PLAYER;
			m_pos.z -= cosf(Rot) * MOVE_PLAYER;
			m_rot.y = (-D3DX_PI * (pInputJoypad->GetLeftAxiz() / D3DX_PI));
		}
		else
		{
	...
	//穴の位置更新
	m_pHole->Setpos(D3DXVECTOR3(m_pos.x, m_pos.y - 5.0f, m_pos.z));
	...
	CollisitionBall(&m_pos, &m_posold, &m_move);
...
void CPlayer::CollisitionBall(D3DXVECTOR3 * pPos, D3DXVECTOR3 * pPosOld, D3DXVECTOR3 * pMove)
{
	CScene *pScene;
	...
	while (pScene != NULL)
	{
		...
		if (pScene->GetDeath() == false)
		{
			...
			{
				//オブジェクトのあたる判定
				bLand = ((CBall*)pScene)->CollisionObject(pPos, pPosOld, pMove);
			}
		}
