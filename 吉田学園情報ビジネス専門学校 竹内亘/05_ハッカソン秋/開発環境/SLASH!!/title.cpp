...
// マクロ定義
//*****************************************************************************
#define TITLE_TEXTURE_NAME  "data/TEXTURE/title000.jpeg" // 読み込むテクスチャファイル名
...
void UpdateTitle(void)
{
	...
	if(GetKeyboardTrigger(DIK_RETURN) && fade == FADE_NONE && *disp == true)
	{// ENTERキーが押された
		SetFade(MODE_TUTORIAL);
		PlaySound(SOUND_LABEL_SE_DECIDE);  // 決定音を流す
	}

	if (g_nCounterTitle % 400 == 0 && fade == FADE_NONE)
	{// 一定時間がたつ
