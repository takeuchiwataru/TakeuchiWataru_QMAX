...
// マクロ定義
//*****************************************************************************
#define BG_TEXTURE_NAME  "data/TEXTURE/bg000.png"      // 読み込むテクスチャファイル名
#define BG_POS_X         (0)                           // 背景の左上Ｘ座標
#define BG_POS_Y         (0)                           // 背景の左上Ｙ座標
#define BG_WIDTH         (SCREEN_WIDTH)                // 背景の幅
#define BG_HEIGHT        (SCREEN_HEIGHT)               // 背景の高さ
#define ANIMATIONPATTERN (0.0025f * g_PatternAnimBG01) // 背景テクスチャ座標を切り替える計算式
...
void UpdateBG01(void)
{
	...
	pVtx[1].tex.x = ANIMATIONPATTERN + 1.0f;
	pVtx[2].tex.x = ANIMATIONPATTERN + 0.0f;
	pVtx[3].tex.x = ANIMATIONPATTERN + 1.0f;
