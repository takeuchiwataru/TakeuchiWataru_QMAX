...
HRESULT CTitleLogo::Init(D3DXVECTOR3 pos)
{
	...
	// 位置の設置
	CLogo::SetPosition(pos, TEXTURE_SIZE_X, TEXTURE_SIZE_Y);
...
HRESULT CResultLogo::Init(D3DXVECTOR3 pos, RESULTTYPE type)
{
	...
	// オブジェクトの種類の設定
	SetObjType(CScene::OBJTYPE_RESULTLOGO);
...
HRESULT CResultLogo::Load(void)
{
	...
	// テクスチャの生成
	D3DXCreateTextureFromFile(pDevice, RESULTLOGO_TEXTURE000, &m_apTexture[0]);
...
HRESULT CTutorialLogo::Init(D3DXVECTOR3 pos)
{
	...
	// オブジェクトの種類の設定
	SetObjType(CScene::OBJTYPE_TUTORIAL);
...
HRESULT CPressEnter::Init(D3DXVECTOR3 pos, PRESSENTER type)
{
	...
	// オブジェクトの種類の設定
	SetObjType(CScene::OBJTYPE_PRESSENTER);
