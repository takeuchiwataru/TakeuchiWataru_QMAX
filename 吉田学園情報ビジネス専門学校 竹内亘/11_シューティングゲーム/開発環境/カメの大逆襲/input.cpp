...
void CInputJoyPad::Update(void)
{
	...
	if (m_pDevice != NULL)
	{
		...
		if (SUCCEEDED(m_pDevice->GetDeviceState(sizeof(DIJOYSTATE), &dijs)))
		{
			...
			//if (m_LeftAxizX > 100 || m_LeftAxizX < -100 || m_LeftAxizY > 100 || m_LeftAxizY < -100)
			//{// 左スティック倒してる時
			//	m_aStick[0] = 0x80;
			//}
			//else 
			...
			//	m_aStick[0] = 0;
			//}
			//if (m_RightAxizX > 100 || m_RightAxizX < -100 || m_RightAxizY > 100 || m_RightAxizY < -100)
			//{// 右スティック倒してる時
			//	m_aStick[1] = 0x80;
			//}
			//else
			...
			else if (m_LeftAxizX > 100 || m_LeftAxizX < -100 || m_LeftAxizY > 100 || m_LeftAxizY < -100)
			{
			...
			else if (m_RightAxizX > 100 || m_RightAxizX < -100 || m_RightAxizY > 100 || m_RightAxizY < -100)
			{
			...
			for (nCntKey = 0; nCntKey < 32; nCntKey++)
			{
				// ジョイパッド入力情報保存(トリガー)
