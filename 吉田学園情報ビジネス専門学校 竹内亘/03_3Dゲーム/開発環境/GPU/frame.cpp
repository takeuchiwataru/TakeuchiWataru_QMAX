...
void InitFrame(void)
{
	...
	// テクスチャの読み込み
	D3DXCreateTextureFromFile(pDevice,				// デバイスへのポインタ
		FRAME_TEXTURENAME3,							// ファイルの名前
		&g_pTextureFrame[2]);						// テクスチャへのポインタ
...
void MakeVertexFrame(LPDIRECT3DDEVICE9 pDevice)
{
		...
		for (int nCntFrame = 0; nCntFrame < MAX_FRAME; nCntFrame++)
		{
			...
			{//タイマーフレーム
				//頂点座標の設定
				pVtx[0].pos = D3DXVECTOR3(540, 0, 0);
				pVtx[1].pos = D3DXVECTOR3(740, 0, 0);
				pVtx[2].pos = D3DXVECTOR3(540, 100, 0);
				pVtx[3].pos = D3DXVECTOR3(740, 100, 0);
			...
			{//スコアフレーム
				//頂点座標の設定
				pVtx[0].pos = D3DXVECTOR3(870, 0, 0);
				pVtx[1].pos = D3DXVECTOR3(1250, 0, 0);
				pVtx[2].pos = D3DXVECTOR3(870, 100, 0);
			...
			else if (nCntFrame == FRAME_LIFE)
			{// ライフフレーム
				pVtx[0].pos = D3DXVECTOR3(10, 0, 0);
				pVtx[1].pos = D3DXVECTOR3(250, 0, 0);
				pVtx[2].pos = D3DXVECTOR3(10, 100, 0);
				pVtx[3].pos = D3DXVECTOR3(250, 100, 0);
...
//	for (int nCntFrame = 0; nCntFrame < MAX_FRAME; nCntFrame++)
//	{
//		if (g_aFrame[nCntFrame].bUse == false)
//		{
//			g_aFrame[nCntFrame].pos = pos;
//
//			//頂点座標の設定
//			pVtx[0].pos = D3DXVECTOR3(g_aFrame[nCntFrame].pos.x - FRAME_SIZE_X, g_aFrame[nCntFrame].pos.y - FRAME_SIZE_Y, g_aFrame[nCntFrame].pos.z);
//			pVtx[1].pos = D3DXVECTOR3(g_aFrame[nCntFrame].pos.x + FRAME_SIZE_X, g_aFrame[nCntFrame].pos.y - FRAME_SIZE_Y, g_aFrame[nCntFrame].pos.z);
...
//			pVtx[3].pos = D3DXVECTOR3(g_aFrame[nCntFrame].pos.x + FRAME_SIZE_X, g_aFrame[nCntFrame].pos.y + FRAME_SIZE_Y, g_aFrame[nCntFrame].pos.z);
//
//			//g_aFrame[nCntFrame].fWidth = fWidth;
//
//			//g_aFrame[nCntFrame].fHeight = fHeight;
//
//			g_aFrame[nCntFrame].bUse = true;	//使用してる
//			break;
//		}
//		pVtx += 4;	//頂点データを４つ分進める
//
